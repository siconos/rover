...
int main(int argc, char* argv[])
{
  try{
    ...
    double g=9.8;
    double ls1,ls2;
    double RAngle[3]= {8000.0,0.0,0.0}; //first component is the radius of sphere, the other two componet is not used in the code
                                        // {Radius of Sphere,Num,Num}
    ...
    SP::Interaction inter5(new Interaction( nslaw5, relation5));
    ...
    Rover3D->nonSmoothDynamicalSystem()->link(inter, rover_ds);
    Rover3D->nonSmoothDynamicalSystem()->link(inter1, rover_ds);
...
//     {fprintf(pFile,"%e %e %e %e\n",PlaneRotationVector[0],PlaneRotationVector[1],PlaneRotationVector[2],PlaneRotationVector[3]);}
    ...
    fprintf(pFile,"geometry Sphere{ radius %e}", RAngle[0]/100);
    fprintf(pFile,"}\n]\n}\n");
