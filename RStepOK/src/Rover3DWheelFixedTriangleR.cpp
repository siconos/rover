  ...
  for(int i = 0; i < 6; ++i){
    ...
    else if(checkIfInsideSegment(Q, vert1, vert2) || checkIfInsideSegment(Q, vert1, vert3) || checkIfInsideSegment(Q, vert2, vert3))
    {
      ...
      if(checkIfInsideSegment(Q, vert1, vert2)){
        ...
        //calculate the projection coords on the segment

        SiconosVector segment1(3);
        SiconosVector segment2(3);
      
        sub(vert2, vert1, segment1);

        sub(P, vert1, segment2);

        double norm = segment1.norm2();

        scal(1.0/norm, segment1, segment1);      
        double inn_prod = inner_prod(segment1, segment2);

        SiconosVector Qproject(3);

        scal(inn_prod, segment1, Qproject);

        //recalculate the normal

        SiconosVector nrmTmp(3);

        sub(P, Qproject, nrmTmp);

        double norm2 = nrmTmp.norm2();

        scal(1.0/norm2, nrmTmp, nrmTmp);

        vectMap.insert(std::pair<double, SiconosVector>(di1, nrmTmp));
      ...
      if(checkIfInsideSegment(Q, vert1, vert3)){
        ...
        vectMap.insert(std::pair<double, SiconosVector>(di2, nrmTmp));
      ...
      if(checkIfInsideSegment(Q, vert2, vert3)){
        ...
        sub(vert2, vert3, segment1);
        sub(P, vert3, segment2);
        ...
        vectMap.insert(std::pair<double, SiconosVector>(di3, nrmTmp));
      
      }
       
      it = min_element(distanceV.begin(), distanceV.end());

      it2 = max_element(distanceV.begin(), distanceV.end());

      //normal update

      jacNorm = vectMap.find((*it))->second;

      jacNorm.setValue(0, -jacNorm.getValue(0));

      if(jacNorm.getValue(2) != 0){

        jacNorm = vectMap.find((*it2))->second;
      ...
      if(((_id%4) == 3) || ((_id%4) == 0)){
        ...
        double t = jacNorm.getValue(0);

        jacNorm.setValue(0, -jacNorm.getValue(1));
        jacNorm.setValue(1, -t);
...
void Rover3DWheelFixedTriangleR::computeJachq(const double time, Interaction& inter)
{
  ...
  if((jacNorm.getValue(0) < 0) && (orthoBase(1,1) > 0)){

    orthoBase(0,1) = -orthoBase(0,1);
    orthoBase(1,1) = -orthoBase(1,1);
    orthoBase(2,1) = -orthoBase(2,1);
