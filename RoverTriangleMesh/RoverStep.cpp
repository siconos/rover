...
int main(int argc, char* argv[])
{
  try
  {
    ...
    for(int j = 0; j < 6; ++j){
        ...
        //forward step
        //relation[j*NumTr+0].reset(new Rover3DWheelFixedTriangleR(500, 0, 0, 0, 0, 500, 1000, 0, 500, j, R));
        //relation[j*NumTr+1].reset(new Rover3DWheelFixedTriangleR(1000, 0, 500, 0, 0, 500, 0, 10, 500, j, R));
        //relation[j*NumTr+2].reset(new Rover3DWheelFixedTriangleR(0, 10, 500, 1000, 5, 500, 1000, 0, 500, j, R));
        //relation[j*NumTr+3].reset(new Rover3DWheelFixedTriangleR(0, 10, 500, 1000, 10, 500, 500, 10, 1000, j, R));

        //backward step
        relation[j*NumTr+0].reset(new Rover3DWheelFixedTriangleR(1000, 10, 500, 0, 10, 500, 500, 10, 0, j, R));
        relation[j*NumTr+1].reset(new Rover3DWheelFixedTriangleR(0, 10, 500, 0, 0, 500, 1000, 0, 500, j, R));
        relation[j*NumTr+2].reset(new Rover3DWheelFixedTriangleR(1000, 0, 500, 1000, 10, 500, 0, 10, 500, j, R));
        relation[j*NumTr+3].reset(new Rover3DWheelFixedTriangleR(0, 0, 500, 1000, 0, 500, 500, 0, 1000, j, R));
    ...
    ydots(k, 1) = inter[0]->y(1)->getValue(0);
    ydots(k, 2) = inter[0]->y(1)->getValue(1);
    ydots(k, 3) = inter[0]->y(1)->getValue(2);

    ydots(k, 4) = inter[1]->y(1)->getValue(0);
    ydots(k, 5) = inter[1]->y(1)->getValue(1);
    ydots(k, 6) = inter[1]->y(1)->getValue(2);

    ydots(k, 10) = inter[2]->y(1)->getValue(0);
    ydots(k, 11) = inter[2]->y(1)->getValue(1);
    ydots(k, 12) = inter[2]->y(1)->getValue(2);

    ydots(k, 16) = inter[3]->y(1)->getValue(0);
    ydots(k, 17) = inter[3]->y(1)->getValue(1);
    ydots(k, 18) = inter[3]->y(1)->getValue(2);
    ...
    while(s->hasNextEvent())
    {
      ...
      //force needed to mount the backward step is 19000
      // 180 if(dataPlot(k, 0) > 30 && dataPlot(k, 0) <= 170){ f = 19000
      if(dataPlot(k, 0) > 30){
      (*Force)(9) = 20000;       //FL traction
      (*Force)(10) = 20000;      //FR  traction
      (*Force)(14) = 20000;      //ML traction
      (*Force)(15) = 20000;      //BL traction
      (*Force)(19) = 20000;      //MR traction
      (*Force)(20) = 20000;     //BR traction
      }


      /*if(dataPlot(k, 0) > 70){
      (*Force)(9) = 40900;       //FL traction
      (*Force)(10) = 40900;      //FR  traction
      (*Force)(14) = 40900;      //ML traction
      (*Force)(15) = 40900;      //BL traction
      (*Force)(19) = 40900;      //MR traction
      (*Force)(20) = 40900;     //BR traction
      }*/

      /*if(dataPlot(k, 0) > 220){
      (*Force)(9) = 3200;       //FL traction
      (*Force)(10) = 3200;      //FR  traction
      (*Force)(14) = 3200;      //ML traction
      (*Force)(15) = 3200;      //BL traction
      (*Force)(19) = 3200;      //MR traction
      (*Force)(20) = 3200;     //BR traction
      }
      */

      ydots(k, 0) = dataPlot(k, 0);
  ...
  double PlaneRotationVector[4];
    ...
    //system("cat ForwStep.wrl normals.wrl RoverModel.wrl data.wrl RoverAnimation.wrl > run.wrl");
    system("cat BackStep.wrl normals.wrl RoverModel.wrl data.wrl RoverAnimation.wrl > run.wrl");
    // --- Output files ---
