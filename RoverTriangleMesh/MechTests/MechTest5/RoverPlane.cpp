...
int main(int argc, char* argv[])
{
  try
  {
    ...
    double T = 150;                 // final computation time
    ...
    vector<SP::Rover3DWheelFixedSphereR> relationS(6*NumTr);

    vector<SP::Interaction> inter(2*6*NumTr);
    ...
    for(int j = 0; j < 6; ++j){
      ...
      for(outerIt = outerVec.begin(), p = 0; outerIt != outerVec.end(); ++outerIt, ++p){
        ...
        relationS[j*NumTr + p].reset(new Rover3DWheelFixedSphereR(j, sRad, 5000, -160, 5000, R));

        inter[j*NumTr + p].reset(new Interaction(allDS, j*NumTr + p, 3, nslaw, relation[j*NumTr + p]));        

        inter[j*NumTr + p + 6].reset(new Interaction(allDS, j*NumTr + p + 6, 3, nslaw, relationS[j*NumTr + p]));        

        allInteractions.insert(inter[j*NumTr + p]);

        allInteractions.insert(inter[j*NumTr + p + 6]);
        ...
        //normals save-off for all wheels 
          ...
          outFile << "\t\t\t point [" << endl;
          outFile << "\t\t\t     " << -(relation[j*NumTr + p]->vert1.z)/100 << " " << (relation[j*NumTr + p]->vert1.y)/100 << " " << -(relation[j*NumTr + p]->vert1.x)/100 << "," << endl;
          outFile << "\t\t\t     " << -(relation[j*NumTr + p]->vert1.z)/100 + (-5)*relation[j*NumTr + p]->n.z << " " << (relation[j*NumTr + p]->vert1.y)/100 + 5*relation[j*NumTr + p]->n.y << " " << -(relation[j*NumTr + p]->vert1.x)/100 + (-5)*relation[j*NumTr + p]->n.x << "," << endl;
          outFile << "\t\t\t ]" << endl;
          outFile << "\t\t }" << endl;
    ...
    outFile << endl << "Transform{" << endl;
    outFile << "\t translation " << -50 << " " << -1.6 <<  " " << -50 << endl;     
    outFile << endl << "\t children[" << endl;
    outFile << endl << "\t Shape{" << endl;
    outFile << endl << "\t\t appearance Appearance{" << endl; 
    outFile << endl <<  "\t\t material Material {} " << endl;
    outFile << endl << "\t}" << endl;
    outFile << "\t\t geometry Sphere {" << endl;
    outFile << "radius " << sRad/100 << endl;
    outFile << "\t }" << endl;
    outFile << "\t}" << endl;
    outFile << "\t]" << endl;
    outFile << "}" << endl;
    ...
    osnspb->numericsSolverOptions()->dparam[0]=1e-8;// Tolerance
    osnspb->numericsSolverOptions()->dparam[2]=1e-8;// Local tolerance
    ...
    yfunction(k, 19) = inter[6]->y(0)->getValue(0);
    yfunction(k, 20) = inter[6]->y(0)->getValue(1);
    yfunction(k, 21) = inter[6]->y(0)->getValue(2);

    yfunction(k, 22) = inter[7]->y(0)->getValue(0);
    yfunction(k, 23) = inter[7]->y(0)->getValue(1);
    yfunction(k, 24) = inter[7]->y(0)->getValue(2);

    yfunction(k, 25) = inter[8]->y(0)->getValue(0);
    yfunction(k, 26) = inter[8]->y(0)->getValue(1);
    yfunction(k, 27) = inter[8]->y(0)->getValue(2);

    yfunction(k, 28) = inter[9]->y(0)->getValue(0);
    yfunction(k, 29) = inter[9]->y(0)->getValue(1);
    yfunction(k, 30) = inter[9]->y(0)->getValue(2);

    yfunction(k, 31) = inter[10]->y(0)->getValue(0);
    yfunction(k, 32) = inter[10]->y(0)->getValue(1);
    yfunction(k, 33) = inter[10]->y(0)->getValue(2);

    yfunction(k, 34) = inter[11]->y(0)->getValue(0);
    yfunction(k, 35) = inter[11]->y(0)->getValue(1);
    yfunction(k, 36) = inter[11]->y(0)->getValue(2);

    SimpleMatrix ydotfunction(N+1, 37);
    ...
    ydotfunction(k, 19) = inter[6]->y(1)->getValue(0);
    ydotfunction(k, 20) = inter[6]->y(1)->getValue(1);
    ydotfunction(k, 21) = inter[6]->y(1)->getValue(2);

    ydotfunction(k, 22) = inter[7]->y(1)->getValue(0);
    ydotfunction(k, 23) = inter[7]->y(1)->getValue(1);
    ydotfunction(k, 24) = inter[7]->y(1)->getValue(2);

    ydotfunction(k, 25) = inter[8]->y(1)->getValue(0);
    ydotfunction(k, 26) = inter[8]->y(1)->getValue(1);
    ydotfunction(k, 27) = inter[8]->y(1)->getValue(2);

    ydotfunction(k, 28) = inter[9]->y(1)->getValue(0);
    ydotfunction(k, 29) = inter[9]->y(1)->getValue(1);
    ydotfunction(k, 30) = inter[9]->y(1)->getValue(2);

    ydotfunction(k, 31) = inter[10]->y(1)->getValue(0);
    ydotfunction(k, 32) = inter[10]->y(1)->getValue(1);
    ydotfunction(k, 33) = inter[10]->y(1)->getValue(2);

    ydotfunction(k, 34) = inter[11]->y(1)->getValue(0);
    ydotfunction(k, 35) = inter[11]->y(1)->getValue(1);
    ydotfunction(k, 36) = inter[11]->y(1)->getValue(2);


    SimpleMatrix lambdas(N+1, 37);
    ...
    lambdas(k, 19) = inter[6]->lambda(1)->getValue(0);
    lambdas(k, 20) = inter[6]->lambda(1)->getValue(1);
    lambdas(k, 21) = inter[6]->lambda(1)->getValue(2);

    lambdas(k, 22) = inter[7]->lambda(1)->getValue(0);
    lambdas(k, 23) = inter[7]->lambda(1)->getValue(1);
    lambdas(k, 24) = inter[7]->lambda(1)->getValue(2);

    lambdas(k, 25) = inter[8]->lambda(1)->getValue(0);
    lambdas(k, 26) = inter[8]->lambda(1)->getValue(1);
    lambdas(k, 27) = inter[8]->lambda(1)->getValue(2);

    lambdas(k, 28) = inter[9]->lambda(1)->getValue(0);
    lambdas(k, 29) = inter[9]->lambda(1)->getValue(1);
    lambdas(k, 30) = inter[9]->lambda(1)->getValue(2);

    lambdas(k, 31) = inter[10]->lambda(1)->getValue(0);
    lambdas(k, 32) = inter[10]->lambda(1)->getValue(1);
    lambdas(k, 33) = inter[10]->lambda(1)->getValue(2);

    lambdas(k, 34) = inter[11]->lambda(1)->getValue(0);
    lambdas(k, 35) = inter[11]->lambda(1)->getValue(1);
    lambdas(k, 36) = inter[11]->lambda(1)->getValue(2);
    ...
    while(s->hasNextEvent())
    {
      ...
      if(dataPlot(k, 0) > 50){
      (*Force)(9) = 5000;       //FL traction
      (*Force)(10) = 5000;      //FR  traction
      (*Force)(14) = 5000;      //ML traction
      (*Force)(15) = 5000;      //BL traction
      (*Force)(19) = 5000;      //MR traction
      (*Force)(20) = 5000;     //BR traction
      }
