...
: LagrangianScleronomousR(), TireMark(WheelMark), TireRadius(WheelRadius), _id(id), vert1(3), vert2(3), vert3(3), planeNorm(3)
{
...
void Rover3DWheelFixedTriangleR::calcDistance(double* dist, double* q)
{
  ...
  for(int i = 0; i < 6; ++i){
    ...
    {  
      ...
      double l = inner_prod(planeNorm, Pvert1);
...
void Rover3DWheelFixedTriangleR::computeh(const double time, Interaction& inter)
{
  ...
  double tmp[6];
  //cout << tmpQ[0] << " " << tmpQ[1] << " " << tmpQ[2] << " " << tmpQ[3]<< " " << tmpQ[4]<< " " << tmpQ[5]<< endl;
...
void Rover3DWheelFixedTriangleR::computeJachq(const double time, Interaction& inter)
{
  ...
  orthoBase(0,0) = planeNorm.getValue(0);
  orthoBase(0,1) = planeNorm.getValue(1);
  orthoBase(0,2) = planeNorm.getValue(2);
