...
int main(int argc, char* argv[])
{
  try
  {
    ...
    double T = 85;                 // final computation time
    ...
    for(int j = 0; j < 6; ++j){
      ...
      //relation[j*NumTr + 1].reset(new Rover3DWheelFixedTriangleR(1010, 5, 0, 1010, 5, 1000, 1000, 0, 0, j, R));
      //relation[j*NumTr + 2].reset(new Rover3DWheelFixedTriangleR(1010, 5, 1000, 1000, 0, 1000, 1000, 0, 0, j, R));
      //relation[j*NumTr + 3].reset(new Rover3DWheelFixedTriangleR(1010, 5, 0, 1010, 5, 1000, 2010, 5, 500, j, R));
      ...
      outFile << "\t\t\t point [" << endl;
      outFile << "\t\t\t     " << -(relation[j*NumTr + 0]->vert1.z)/100 << " " << (relation[j*NumTr + 0]->vert1.y)/100 << " " << -(relation[j*NumTr + 0]->vert1.x)/100 << "," << endl;
      outFile << "\t\t\t     " << -(relation[j*NumTr + 0]->vert1.z)/100 + (-5)*relation[j*NumTr + 0]->n.z << " " << (relation[j*NumTr + 0]->vert1.y)/100 + 5*relation[j*NumTr + 0]->n.y << " " << -(relation[j*NumTr + 0]->vert1.x)/100 + (-5)*relation[j*NumTr + 0]->n.x << "," << endl;
      outFile << "\t\t\t ]" << endl;
      outFile << "\t\t }" << endl;
      ...
      outFile << "\t\t coord Coordinate { " << endl;
      outFile << "\t\t\t point [" << endl;
      outFile << "\t\t\t     " << -(relation[j*NumTr + 1]->vert1.z)/100 << " " << (relation[j*NumTr + 1]->vert1.y)/100 << " " << -(relation[j*NumTr + 1]->vert1.x)/100 << "," << endl;
      outFile << "\t\t\t     " << -(relation[j*NumTr + 1]->vert1.z)/100 + (-5)*relation[j*NumTr + 1]->n.z << " " << (relation[j*NumTr + 1]->vert1.y)/100 + 5*relation[j*NumTr + 1]->n.y << " " << -(relation[j*NumTr + 1]->vert1.x)/100 + (-5)*relation[j*NumTr + 1]->n.x << "," << endl;
      outFile << "\t\t\t ]" << endl;
      outFile << "\t\t }" << endl;
      outFile << "\t\t color Color { " << endl;
      outFile << "\t\t\t color [ " << endl;
      ...
      outFile << "\t\t coord Coordinate { " << endl;
      outFile << "\t\t\t point [" << endl;
      outFile << "\t\t\t     " << -(relation[j*NumTr + 2]->vert1.z)/100 << " " << (relation[j*NumTr + 2]->vert1.y)/100 << " " << -(relation[j*NumTr + 2]->vert1.x)/100 << "," << endl;
      outFile << "\t\t\t     " << -(relation[j*NumTr + 2]->vert1.z)/100 + (-5)*relation[j*NumTr + 2]->n.z << " " << (relation[j*NumTr + 2]->vert1.y)/100 + 5*relation[j*NumTr + 2]->n.y << " " << -(relation[j*NumTr + 2]->vert1.x)/100 + (-5)*relation[j*NumTr + 2]->n.x << "," << endl;
      outFile << "\t\t\t ]" << endl;
      outFile << "\t\t }" << endl;
      outFile << "\t\t color Color { " << endl;
      outFile << "\t\t\t color [ " << endl;
      ...
      outFile << "\t\t coord Coordinate { " << endl;
      outFile << "\t\t\t point [" << endl;
      outFile << "\t\t\t     " << -(relation[j*NumTr + 3]->vert1.z)/100 << " " << (relation[j*NumTr + 3]->vert1.y)/100 << " " << -(relation[j*NumTr + 3]->vert1.x)/100 << "," << endl;
      outFile << "\t\t\t     " << -(relation[j*NumTr + 3]->vert1.z)/100 + (-5)*relation[j*NumTr + 3]->n.z << " " << (relation[j*NumTr + 3]->vert1.y)/100 + 5*relation[j*NumTr + 3]->n.y << " " << -(relation[j*NumTr + 3]->vert1.x)/100 + (-5)*relation[j*NumTr + 3]->n.x << "," << endl;
      outFile << "\t\t\t ]" << endl;
      outFile << "\t\t }" << endl;
      outFile << "\t\t color Color { " << endl;
      outFile << "\t\t\t color [ " << endl;
    ...
    SimpleMatrix dataPlot1(N + 1, 3);
    SimpleMatrix dataPlot2(N + 1, 3);
    SimpleMatrix dataPlot3(N + 1, 3);
    SimpleMatrix dataPlot4(N + 1, 3);
    SimpleMatrix dataPlot5(N + 1, 3);
    SimpleMatrix dataPlot6(N + 1, 3);
    ...
    dataPlot1(k, 0) = (*q)(0)/100;
    dataPlot1(k, 1) = (*q)(1)/100;
    dataPlot1(k, 2) = (*q)(2)/100;
    dataPlot2(k, 0) = (*q)(0)/100;
    dataPlot2(k, 1) = (*q)(1)/100;
    dataPlot2(k, 2) = (*q)(2)/100;
    dataPlot3(k, 0) = (*q)(0)/100;
    dataPlot3(k, 1) = (*q)(1)/100;
    dataPlot3(k, 2) = (*q)(2)/100;
    dataPlot4(k, 0) = (*q)(0)/100;
    dataPlot4(k, 1) = (*q)(1)/100;
    dataPlot4(k, 2) = (*q)(2)/100;
    dataPlot5(k, 0) = (*q)(0)/100;
    dataPlot5(k, 1) = (*q)(1)/100;
    dataPlot5(k, 2) = (*q)(2)/100;
    dataPlot6(k, 0) = (*q)(0)/100;
    dataPlot6(k, 1) = (*q)(1)/100;
    dataPlot6(k, 2) = (*q)(2)/100;
    ...
    while(s->hasNextEvent())
    {
      ...
      //set to 17000 with step height equal to 3

      if(dataPlot(k, 0) >= 50){

        (*Force)(9) = 15000;       //FL traction
        (*Force)(10) = 15000;      //FR  traction
        (*Force)(14) = 15000;      //ML traction
        (*Force)(15) = 15000;      //BL traction
        (*Force)(19) = 15000;      //MR traction
        (*Force)(20) = 15000;      //BR traction
      ...
      dataPlot1(k, 0) = (relation[0]->Pj.x)/100;
      dataPlot1(k, 1) = (relation[0]->Pj.y)/100;
      dataPlot1(k, 2) = (relation[0]->Pj.z)/100;
      dataPlot2(k, 0) = (relation[4]->Pj.x)/100;
      dataPlot2(k, 1) = (relation[4]->Pj.y)/100;
      dataPlot2(k, 2) = (relation[4]->Pj.z)/100;
      dataPlot3(k, 0) = (relation[8]->Pj.x)/100;
      dataPlot3(k, 1) = (relation[8]->Pj.y)/100;
      dataPlot3(k, 2) = (relation[8]->Pj.z)/100;
      dataPlot4(k, 0) = (relation[12]->Pj.x)/100;
      dataPlot4(k, 1) = (relation[12]->Pj.y)/100;
      dataPlot4(k, 2) = (relation[12]->Pj.z)/100;
      dataPlot5(k, 0) = (relation[16]->Pj.x)/100;
      dataPlot5(k, 1) = (relation[16]->Pj.y)/100;
      dataPlot5(k, 2) = (relation[16]->Pj.z)/100;
      dataPlot6(k, 0) = (relation[20]->Pj.x)/100;
      dataPlot6(k, 1) = (relation[20]->Pj.y)/100;
      dataPlot6(k, 2) = (relation[20]->Pj.z)/100;
      ...
      yfunction(k, 4) = inter[4]->y(0)->getValue(0);
      yfunction(k, 5) = inter[4]->y(0)->getValue(1);
      yfunction(k, 6) = inter[4]->y(0)->getValue(2);

      yfunction(k, 7) = inter[8]->y(0)->getValue(0);
      yfunction(k, 8) = inter[8]->y(0)->getValue(1);
      yfunction(k, 9) = inter[8]->y(0)->getValue(2);

      yfunction(k, 10) = inter[12]->y(0)->getValue(0);
      yfunction(k, 11) = inter[12]->y(0)->getValue(1);
      yfunction(k, 12) = inter[12]->y(0)->getValue(2);

      yfunction(k, 13) = inter[16]->y(0)->getValue(0);
      yfunction(k, 14) = inter[16]->y(0)->getValue(1);
      yfunction(k, 15) = inter[16]->y(0)->getValue(2);

      yfunction(k, 16) = inter[20]->y(0)->getValue(0);
      yfunction(k, 17) = inter[20]->y(0)->getValue(1);
      yfunction(k, 18) = inter[20]->y(0)->getValue(2);
  ...
  double PlaneRotationVector[4];
    ...
    //contact point animation

    FILE * spFile;
    spFile = fopen("AnimSphere.wrl","w");

    fprintf(spFile,"DEF Animation Group {\n  children [\n");
    fprintf(spFile,"\n");

    fprintf(spFile,"DEF FLWheelInterp PositionInterpolator{\n");
    fprintf(spFile,"    key [ 0 ");
    for (int cmp =1;cmp <= N;cmp++){
      fprintf(spFile,",%e",(dataPlot(cmp,0)-t0)/(T-t0));
    }
    fprintf(spFile,"]\n"); 
    fprintf(spFile,"    keyValue [  ");
    fprintf(spFile,"%e %e %e,\n",dataPlot1(0,0),dataPlot1(0,1),dataPlot1(0,2));
    for (int cmp =1;cmp <= N;cmp++){
      fprintf(spFile,"%e %e %e,\n",-dataPlot1(cmp,2),dataPlot1(cmp,1),-dataPlot1(cmp,0));
    }
    fprintf(spFile,"  ]\n}\n");

    fprintf(spFile,"DEF FRWheelInterp PositionInterpolator{\n");
    fprintf(spFile,"    key [ 0 ");
    for (int cmp =1;cmp <= N;cmp++){
      fprintf(spFile,",%e",(dataPlot(cmp,0)-t0)/(T-t0));
    }
    fprintf(spFile,"]\n"); 
    fprintf(spFile,"    keyValue [  ");
    fprintf(spFile,"%e %e %e,\n",dataPlot2(0,0),dataPlot2(0,1),dataPlot2(0,2));
    for (int cmp =1;cmp <= N;cmp++){
      fprintf(spFile,"%e %e %e,\n",-dataPlot2(cmp,2),dataPlot2(cmp,1),-dataPlot2(cmp,0));
    }
    fprintf(spFile,"  ]\n}\n");

    fprintf(spFile,"DEF CLWheelInterp PositionInterpolator{\n");
    fprintf(spFile,"    key [ 0 ");
    for (int cmp =1;cmp <= N;cmp++){
      fprintf(spFile,",%e",(dataPlot(cmp,0)-t0)/(T-t0));
    }
    fprintf(spFile,"]\n"); 
    fprintf(spFile,"    keyValue [  ");
    fprintf(spFile,"%e %e %e,\n",dataPlot3(0,0),dataPlot3(0,1),dataPlot3(0,2));
    for (int cmp =1;cmp <= N;cmp++){
      fprintf(spFile,"%e %e %e,\n",-dataPlot3(cmp,2),dataPlot3(cmp,1),-dataPlot3(cmp,0));
    }
    fprintf(spFile,"  ]\n}\n");

    fprintf(spFile,"DEF RLWheelInterp PositionInterpolator{\n");
    fprintf(spFile,"    key [ 0 ");
    for (int cmp =1;cmp <= N;cmp++){
      fprintf(spFile,",%e",(dataPlot(cmp,0)-t0)/(T-t0));
    }
    fprintf(spFile,"]\n"); 
    fprintf(spFile,"    keyValue [  ");
    fprintf(spFile,"%e %e %e,\n",dataPlot4(0,0),dataPlot4(0,1),dataPlot4(0,2));
    for (int cmp =1;cmp <= N;cmp++){
      fprintf(spFile,"%e %e %e,\n",-dataPlot4(cmp,2),dataPlot4(cmp,1),-dataPlot4(cmp,0));
    }
    fprintf(spFile,"  ]\n}\n");

    fprintf(spFile,"DEF CRWheelInterp PositionInterpolator{\n");
    fprintf(spFile,"    key [ 0 ");
    for (int cmp =1;cmp <= N;cmp++){
      fprintf(spFile,",%e",(dataPlot(cmp,0)-t0)/(T-t0));
    }
    fprintf(spFile,"]\n"); 
    fprintf(spFile,"    keyValue [  ");
    fprintf(spFile,"%e %e %e,\n",dataPlot5(0,0),dataPlot5(0,1),dataPlot5(0,2));
    for (int cmp =1;cmp <= N;cmp++){
      fprintf(spFile,"%e %e %e,\n",-dataPlot5(cmp,2),dataPlot5(cmp,1),-dataPlot5(cmp,0));
    }
    fprintf(spFile,"  ]\n}\n");

    fprintf(spFile,"DEF RRWheelInterp PositionInterpolator{\n");
    fprintf(spFile,"    key [ 0 ");
    for (int cmp =1;cmp <= N;cmp++){
      fprintf(spFile,",%e",(dataPlot(cmp,0)-t0)/(T-t0));
    }
    fprintf(spFile,"]\n"); 
    fprintf(spFile,"    keyValue [  ");
    fprintf(spFile,"%e %e %e,\n",dataPlot6(0,0),dataPlot6(0,1),dataPlot6(0,2));
    for (int cmp =1;cmp <= N;cmp++){
      fprintf(spFile,"%e %e %e,\n",-dataPlot6(cmp,2),dataPlot6(cmp,1),-dataPlot6(cmp,0));
    }
    fprintf(spFile,"  ]\n}\n");
    ...
    system("cat output.wrl normals.wrl sphere.wrl RoverModel.wrl data.wrl AnimSphere.wrl RoverAnimation.wrl RoutesSphere.wrl > runSP.wrl");
